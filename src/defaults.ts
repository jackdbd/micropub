...
// It makes sense for the secure session cookie to have an expiration time at
// least equal to that of the refresh token. If the refresh token expired
// earlier than the session cookie, its lifetime would essentially be capped by
// the session expiration.
export const SECURE_SESSION_EXPIRATION = 30 * 24 * 60 * 60 // in seconds
...
export const USE_DEVELOPMENT_ERROR_HANDLER =
  process.env.NODE_ENV === 'development' ? true : false

export const USE_SECURE_FLAG_FOR_SESSION_COOKIE =
  process.env.NODE_ENV === 'production' ? true : false

// export const USE_SOFT_DELETE = false
