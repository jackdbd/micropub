...
import type { IssueAccessToken } from './schemas.js'

export interface Config {
  /**
   * Function that performs the effect of persisting the token to some storage
   * (e.g. storing it in a database).
   */
...
export const defIssueAccessToken = (config: Config) => {
  ...
  const issueAccessToken: IssueAccessToken = async (payload) => {
