...
// JSON Schema (and, by extension, ajv) is designed to work with JSON-compatible
// data, which does not include functions. So I would need to validate this
// function separately. For now I'm just using this schema to generate the type
// definition.

const RETRIEVE_AUTHORIZATION_CODE_DESCRIPTION = `Function that retrieves an 
authorization code from a storage backend.`

const retrieveAuthorizationCode_ = Type.Function(
  [authorization_code],
  Type.Promise(
    Type.Union([
      authorization_code_immutable_record,
  ...
  ),
  {
    ...
    description: RETRIEVE_AUTHORIZATION_CODE_DESCRIPTION
  }
)
...
// export type RetrieveAuthorizationCode = (
//   code: string
// ) => Promise<AuthorizationCodeImmutableRecord | AuthorizationCodeMutableRecord>

const retrieveAuthorizationCode = Type.Any({
  title: 'retrieveAuthorizationCode',
  description: RETRIEVE_AUTHORIZATION_CODE_DESCRIPTION
})

const ON_AUTHORIZATION_CODE_VERIFIED_DESCRIPTION = `Handler that runs after the 
authorization code has been verified. You should use this handler to inform your
storage backend that the authorization code has been used.`

const onAuthorizationCodeVerified_ = Type.Function(
  [authorization_code],
  Type.Promise(Type.Void()),
  {
    title: 'onAuthorizationCodeVerified',
    description: ON_AUTHORIZATION_CODE_VERIFIED_DESCRIPTION
  }
)

const onAuthorizationCodeVerified = Type.Any({
  title: 'onAuthorizationCodeVerified',
  description: ON_AUTHORIZATION_CODE_VERIFIED_DESCRIPTION
})
...
export type OnAuthorizationCodeVerified = Static<
  typeof onAuthorizationCodeVerified_
>

// export type OnAuthorizationCodeVerified = (code: string) => Promise<void>

const ON_USER_APPROVED_DESCRIPTION = `Handler executed after the user approves
the authorization request on the consent screen. It should be used to persist
the authorization code generated by the authorization endpoint into your storage
backend.`

const onUserApprovedRequest_ = Type.Function(
  [authorization_code_props],
  Type.Promise(Type.Void()),
  {
    title: 'onUserApprovedRequest',
    description: ON_USER_APPROVED_DESCRIPTION
  }
)

const onUserApprovedRequest = Type.Any({
  title: 'onUserApprovedRequest',
  description: ON_USER_APPROVED_DESCRIPTION
})

/**
 * This handler is executed after the user approves the authorization request on
 * the consent screen. It should be used to persist the authorization code
 * generated by the authorization endpoint into your storage backend.
 */
export type OnUserApprovedRequest = Static<typeof onUserApprovedRequest_>

// export type OnUserApproved = (
//   props: AuthorizationCodeProps
// ) => Promise<void>
