 ...
 */
export const defIndieAuthStart = (config: Config) => {
  const { code_verifier_length, log_prefix, redirect_uri } = config

  const authStart: RouteHandler<RouteGeneric> = async (request, reply) => {
    ...
    // Calling generateCsrf() sets the CSRF token in the session automatically.
    // However, the CSRF token I obtain in the authorization callback's query
    // string is different. I'm not sure why, but if I set the CSRF token
    // manually in the session I don't have this problem.
    // https://github.com/fastify/csrf-protection/blob/7d38e590c64b81e461f66269209370915acfe3e1/index.js#L94
    request.session.set('state', state)
    request.log.debug(
      `${log_prefix}generated state (CSRF token) and set it in session`
    )

    // TODO: check if client_id is registered before proceeding?

    // TODO: User authenticates and approves the request. Authorization endpoint
    // issues authorization code, builds redirect back to client.
